...
export type Sig<T = any> = Signal<T> | Computed<T> | TSignal<any>;
type ExternalStore = <T>(signal: Sig<T>) => T;
export type TRMapSignal<T> = TSignal<T, { c: React.JSX.Element }>;
export type TRSignal<T> = Signal<T> & { c: React.JSX.Element };
...
export function useSignal<T>(initialValue: T): TRSignal<T> {
  ...
  if (!sigRef.current) {
    sigRef.current = new Signal(initialValue, () => middleware());
...
export function useSignalMap<T extends any[]>(initialValue: T): TRMapSignal<T> {
  const [middleware, externalStore] = useSignalListener();
